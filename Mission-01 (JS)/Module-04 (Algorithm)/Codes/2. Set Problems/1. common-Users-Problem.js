...
const idListA = new Set(usersA.map((userA) => { return userA.id }));  // We are storing id instead of the whole object bcz set compares the objects by reference so we will not get the correct output 

let commonUserCount = 0;

const commonUserList = usersB.map((userB) => {
    // Lookup: O(1)
...
console.log("The Number of Common User", commonUserCount);
